...
class AuthServices {
  ...
  //* SIGNUP
  static async signup(userData,req, next) {
    const {firstName,lastName,email,phone,password,passwordConfirm,role}=userData
    let user;
  ...
  static async login(userData, next) {
    ...
    if (user.activated === false)
      return next(
        new AppError(
          `Plz Activate your email/number by then Link sent to your email ${user.email}`,
          401
        )
      );
